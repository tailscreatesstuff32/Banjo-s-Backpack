...
namespace BanjoKazooieLevelEditor
{
  public class GeneralSettings : Form
  {
      ...
      List<LevelEntryPoint> everyEntry_)
    {
      ...
      {
        this.levels[index2] = this.FCF698[index3];
        index3 += 4;
      }
    ...
    private void ripSkyBoxs()
    {
      for (int index = 34736; index < 35735; index += 40)
      {
        ...
        int num8 = (int) num4;
        int num9 = (int) num6;
        this.bkSkyBoxs.Add(new BKSkyBox(sceneID_, skyBoxModel_, secondarySkyBoxModel_, thirdSkyBoxModel_, (short) num7, (short) num8, (short) num9));
      }
    }
    ...
    public void ripLevelNames()
    {
      string levelName_ = "";
      int start_ = 0;
      for (int end_ = 86068; end_ < 86302; ++end_)
      {
        if (this.F9CAE0[end_] == (byte) 0 && levelName_.Length > 0)
        {
          this.bklevelnames.Add(new BKLevelName(levelName_, start_, end_));
          levelName_ = "";
        }
        else if (this.F9CAE0[end_] != (byte) 0)
        {
          if (levelName_ == "")
            start_ = end_;
          levelName_ += ((char) this.F9CAE0[end_]).ToString();
        }
      }
    }
    ...
    private void sceneAssignment_update_Click(object sender, EventArgs e)
    {
      this.updatedF9CAE0 = true;
    }
    ...
    private void updateMusic_btn_Click(object sender, EventArgs e)
    {
      this.updatedF9CAE0 = true;
    }
    ...
    private void skymap_gv_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
    {
        ...
        if (newMap != "undef")
        {
          ...
          for (int index1 = 34736; index1 < 35735 && !flag1; index1 += 40)
          {
            if ((int) this.F9CAE0[index1] * 256 + (int) this.F9CAE0[index1 + 1] == this.bkSkyBoxs[e.RowIndex].sceneID)
            {
              ...
              for (int index2 = 0; index2 < this.bkSkyBoxs.Count; ++index2)
              {
                if (num2 == this.bkSkyBoxs[index2].sceneID)
                {
      ...
      else if (e.ColumnIndex == 2 || e.ColumnIndex == 4 || e.ColumnIndex == 6)
      {
        ...
        if (e.ColumnIndex == 4)
        {
          SkyboxScale skyboxScale2 = new SkyboxScale((int) this.bkSkyBoxs[e.RowIndex].scaleSB2);
          int num = (int) skyboxScale2.ShowDialog();
          short size = skyboxScale2.size;
        ...
        if (e.ColumnIndex == 6)
        {
          SkyboxScale skyboxScale2 = new SkyboxScale((int) this.bkSkyBoxs[e.RowIndex].scaleSB3);
          int num = (int) skyboxScale2.ShowDialog();
          short size = skyboxScale2.size;
      ...
      {
        int num1 = (int) this.sbrf.ShowDialog();
        string newBox = this.sbrf.newBox;
        if (newBox != "undef")
        {
          int num2 = 0;
          for (int index1 = 0; index1 < this.bkSkyBoxDigits.Count<BkSkyBoxDigit>() && num2 == 0; ++index1)
          {
            if (newBox == this.bkSkyBoxDigits[index1].name)
            {
              num2 = this.bkSkyBoxDigits[index1].id;
              if (e.ColumnIndex == 1)
              {
                this.bkSkyBoxs[e.RowIndex].SB1Name = newBox;
                this.bkSkyBoxs[e.RowIndex].skyBoxModel = num2;
              }
              else if (e.ColumnIndex == 3)
              {
                this.bkSkyBoxs[e.RowIndex].SB2Name = newBox;
                this.bkSkyBoxs[e.RowIndex].secondarySkyBoxModel = num2;
              }
              else if (e.ColumnIndex == 5)
              {
                this.bkSkyBoxs[e.RowIndex].SB3Name = newBox;
                this.bkSkyBoxs[e.RowIndex].thirdSkyBoxModel = num2;
              }
              bool flag = false;
              for (int index2 = 34736; index2 < 35735 && !flag; index2 += 40)
              {
                if ((int) this.F9CAE0[index2] * 256 + (int) this.F9CAE0[index2 + 1] == this.bkSkyBoxs[e.RowIndex].sceneID)
                {
                  if (e.ColumnIndex == 1)
                  {
                    this.F9CAE0[index2 + 4] = (byte) (num2 >> 8);
                    this.F9CAE0[index2 + 5] = (byte) num2;
                    this.skyMapTable.Rows[e.RowIndex].BeginEdit();
                  ...
                  {
                    this.F9CAE0[index2 + 12] = (byte) 63;
                    this.F9CAE0[index2 + 13] = (byte) 0;
                    this.F9CAE0[index2 + 16] = (byte) (num2 >> 8);
                    this.F9CAE0[index2 + 17] = (byte) num2;
                    this.skyMapTable.Rows[e.RowIndex].BeginEdit();
                  ...
                  {
                    this.F9CAE0[index2 + 28] = (byte) (num2 >> 8);
                    this.F9CAE0[index2 + 29] = (byte) num2;
                    this.skyMapTable.Rows[e.RowIndex].BeginEdit();
    ...
    private void puzzel_tb_Leave(object sender, EventArgs e)
    {
