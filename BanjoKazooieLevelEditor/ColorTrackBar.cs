...
namespace BanjoKazooieLevelEditor
{
  ...
  public class ColorTrackBar : Control
  {
    ...
    public int Minimum
    {
      set
      {
        if (value >= this.barMaximum)
          throw new ArgumentException("'" + (object) value + "' is not a valid value for 'Mimimum'.\n'Minimum' must be less than 'Maximum'.");
    ...
    public Poles MaximumValueSide
    {
      ...
      set
      {
        if (value == Poles.Top || value == Poles.Bottom)
          throw new ArgumentException("Since your Orientation is set to Horizontal, you can only select Left or Right for this property");
        this.maxSide = value;
        this.trackRect = Rectangle.Empty;
        this.Invalidate();
      }
    ...
    {
      return (ushort) (value & (uint) ushort.MaxValue);
    }
