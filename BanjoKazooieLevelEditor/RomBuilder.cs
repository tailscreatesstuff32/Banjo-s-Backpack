  ...
  internal class RomBuilder
  {
    ...
    public void GVFix(string file)
    {
      ...
      if (num1 == num2 || num2 == 0)
      {
        ...
        for (int index2 = index1; index2 < buffer1.Length; ++index2)
          buffer2[index2 + 24] = buffer1[index2];
        FileStream fileStream = File.Create(Application.StartupPath + "//out//gv-b.bin");
        BinaryWriter binaryWriter = new BinaryWriter((Stream) fileStream);
    ...
    public void extendRom()
    {
      ...
      for (int index2 = num1 + num7; index2 < 67108864; ++index2)
    ...
    public int ByteArrayToInt32(byte[] b_)
    {
      return (int) b_[0] * 16777216 + (int) b_[1] * 65536 + (int) b_[2] * 256 + (int) b_[3];
    }
    ...
    public void updateAllPointers(int index, int difference, bool special)
    {
        ...
        for (int index1 = 0; index1 < this.eorPointers.Count; ++index1)
        {
          ...
          if (pntr1 > index)
          {
            ...
            if (pntr2 % 8 != 0)
            {
              ...
              for (int index2 = pntr2; index2 < pntr2 + num1; ++index2)
                this.rom[index2] = (byte) 0;
              pntr2 += num1;
              difference += num1;
            }
      ...
      {
        for (int index1 = index + 8; index1 < 68816; index1 += 8)
        {
          int num1 = (int) this.rom[index1] * 16777216 + (int) this.rom[index1 + 1] * 65536 + (int) this.rom[index1 + 2] * 256 + (int) this.rom[index1 + 3];
          if (index1 < 68816)
          {
