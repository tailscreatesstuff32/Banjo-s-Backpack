...
namespace BanjoKazooieLevelEditor
{
  internal class ObjImporter
  {
      ...
      for (int index = 0; index < strArray.Length; ++index)
      {
        if (Regex.IsMatch(strArray[index], "^v "))
        {
          ...
          if (match.Success)
            ...
            {
              Convert.ToInt32(Convert.ToDouble(match.Groups[1].Value) * (double) this.vertScale),
              Convert.ToInt32(Convert.ToDouble(match.Groups[2].Value) * (double) this.vertScale),
              Convert.ToInt32(Convert.ToDouble(match.Groups[3].Value) * (double) this.vertScale)
            });
        }
    ...
    private void RipTexture5551(string texture, int index)
    {
      ...
      int index4 = 0;
      for (int index3 = 0; index3 < numArray.Length; index3 += 4)
      {
        byte num1 = (byte) ((int) numArray[index3] >> 3 << 3);
        int num2 = (int) (byte) ((int) numArray[index3 + 1] >> 3 << 3);
        byte num3 = (byte) ((int) numArray[index3 + 2] >> 3 << 3);
        byte num4;
        if (numArray[index3 + 3] == (byte) 0)
        {
          num4 = (byte) 0;
    ...
    private void CalcPrecision(int attemptNo)
    {
      ...
      for (int index1 = 0; index1 < this.alphaVerts.Count && !flag; ++index1)
      {
        for (int index2 = 0; index2 < 3 && !flag; ++index2)
        {
          if (this.alphaVerts[index1][index2] >= (int) short.MaxValue || this.alphaVerts[index1][index2] <= -32767)
          {
            this.vertScale = 1f / (float) attemptNo;
            flag = true;
          }
      ...
      for (int index1 = 0; index1 < this.verts.Count & flag; ++index1)
      {
        for (int index2 = 0; index2 < 3; ++index2)
          this.verts[index1][index2] = (int) ((double) this.vertsO[index1][index2] / (double) this.originalScale * (double) this.vertScale);
      }
      for (int index1 = 0; index1 < this.alphaVerts.Count & flag; ++index1)
      {
        for (int index2 = 0; index2 < 3; ++index2)
          this.alphaVerts[index1][index2] = (int) ((double) this.alphaVertsO[index1][index2] / (double) this.originalScale * (double) this.vertScale);
      }
      if (!flag)
        return;
      ++attemptNo;
      ...
      for (int index1 = 0; index1 < this.faceVs.Count; ++index1)
      {
        ...
        for (int index2 = 0; index2 < TextureSettingsAlpha.Count<TextureSetting>() && !flag1; ++index2)
        {
          if (TextureSettingsAlpha[index2].name == faceTexture)
          {
            ...
            new List<int>((IEnumerable<int>) numArray4).Sort();
            for (int index3 = 0; index3 < 2; ++index3)
            {
              if (numArray4[index3] > numArray4[index3 + 1])
              {
                int num2 = numArray4[index3];
                numArray4[index3] = numArray4[index3 + 1];
                numArray4[index3 + 1] = num2;
                index3 = 0;
              }
            }
      ...
      string name)
    {
      ...
      for (int index2 = 0; index2 < numArray6.Length; ++index2)
        numArray6[index2] = bytesInFile[num1 + index2];
      byte[] numArray7 = new byte[width * height * 4];
      int index3 = 0;
      for (int index2 = 0; index2 < numArray6.Length; ++index2)
      {
        byte num2 = (byte) ((uint) numArray6[index2] >> 4);
        byte num3 = (byte) ((uint) (byte) ((uint) numArray6[index2] << 4) >> 4);
        numArray7[index3] = numArray2[(int) num2];
        numArray7[index3 + 1] = numArray3[(int) num2];
        numArray7[index3 + 2] = numArray4[(int) num2];
        numArray7[index3 + 3] = numArray5[(int) num2];
        numArray7[index3 + 4] = numArray2[(int) num3];
        numArray7[index3 + 5] = numArray3[(int) num3];
        numArray7[index3 + 6] = numArray4[(int) num3];
        numArray7[index3 + 7] = numArray5[(int) num3];
        index3 += 8;
      }
      byte[] source = new byte[numArray7.Length];
      for (int index2 = 0; index2 < numArray7.Length; index2 += 4)
      {
        source[index2] = numArray7[index2 + 2];
        source[index2 + 1] = numArray7[index2 + 1];
        source[index2 + 2] = numArray7[index2];
        source[index2 + 3] = numArray7[index2 + 3];
      }
