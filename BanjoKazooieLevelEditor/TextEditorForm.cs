...
namespace BanjoKazooieLevelEditor
{
  public class TextEditorForm : Form
  {
    ...
    {
      this.tpf.ProgressValue = e.ProgressPercentage;
    }

    private void worker_DoWork(object sender, DoWorkEventArgs e)
    {
      this.ripAllTextFiles();
    }
    ...
    private void save()
    {
      ...
      {
        int num2 = (int) MessageBox.Show("Could not save file");
      }
    }
      ...
      if (file[0] == (byte) 1 && file[1] == (byte) 3 && file[2] == (byte) 0)
      {
        ...
        while (index1 < file.Length)
        {
          if (file[index1] == (byte) 6 && file[index1 + 1] == (byte) 1 && file[index1 + 2] == (byte) 0)
          {
            ++group_;
            int index2 = index1 + 3;
            byte num3 = file[index2];
            int index3 = index2 + 1;
            byte num4 = file[index3];
            index1 = index3 + 1;
            byte[] hex_ = new byte[(int) num4];
            int num5 = index1 + (int) num4;
            int index4 = 0;
            for (; index1 < num5; ++index1)
            {
              hex_[index4] = file[index1];
              ++index4;
            }
            TextFilePart textFilePart = new TextFilePart(hex_, (int) num3, top_, false, true, group_);
          ...
          {
            byte num3 = file[index1];
            int index2 = index1 + 1;
            byte num4 = file[index2];
            index1 = index2 + 1;
            byte[] hex_ = new byte[(int) num4];
            int num5 = index1 + (int) num4;
            int index3 = 0;
            for (; index1 < num5; ++index1)
            {
              hex_[index3] = file[index1];
              ++index3;
            }
            TextFilePart textFilePart = new TextFilePart(hex_, (int) num3, top_, false, false, group_);
            textFilePartList.Add(textFilePart);
          }
          else
            ++index1;
